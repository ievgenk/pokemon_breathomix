const gluttonyAbillity = {
  effect_changes: [],
  effect_entries: [
    {
      effect:
        'Pokémon mit dieser Fähigkeit essen eine getragene Beere, wenn sie weniger als die Hälfte ihrer maximalen hp haben, unabhängig von der Auslöseschwelle der Beere.',
      language: { name: 'de', url: 'https://pokeapi.co/api/v2/language/6/' },
      short_effect:
        'Lässt das Pokémon eine getragene Beere essen, wenn es weniger als die Hälfte seiner maximalen hp hat.'
    },
    {
      effect:
        "This Pokémon eats any held Berry triggered by low HP when it falls below 50% of its HP, regardless of the Berry's usual threshold.",
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      short_effect:
        'Makes the Pokémon eat any held Berry triggered by low HP below 1/2 its max HP.'
    }
  ],
  flavor_text_entries: [
    {
      flavor_text: 'Encourages the early use\nof a held Berry.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'diamond-pearl',
        url: 'https://pokeapi.co/api/v2/version-group/8/'
      }
    },
    {
      flavor_text: 'Encourages the early use\nof a held Berry.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'platinum',
        url: 'https://pokeapi.co/api/v2/version-group/9/'
      }
    },
    {
      flavor_text: 'Encourages the early use\nof a held Berry.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'heartgold-soulsilver',
        url: 'https://pokeapi.co/api/v2/version-group/10/'
      }
    },
    {
      flavor_text: 'Encourage l’utilisation\nd’une Baie tenue.',
      language: { name: 'fr', url: 'https://pokeapi.co/api/v2/language/5/' },
      version_group: {
        name: 'black-white',
        url: 'https://pokeapi.co/api/v2/version-group/11/'
      }
    },
    {
      flavor_text: 'Encourages the early use\nof a held Berry.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'black-white',
        url: 'https://pokeapi.co/api/v2/version-group/11/'
      }
    },
    {
      flavor_text: 'Encourages the early use\nof a held Berry.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'black-2-white-2',
        url: 'https://pokeapi.co/api/v2/version-group/14/'
      }
    },
    {
      flavor_text: 'きのみを　いつもより\nはやく　つかう。',
      language: {
        name: 'ja-Hrkt',
        url: 'https://pokeapi.co/api/v2/language/1/'
      },
      version_group: {
        name: 'x-y',
        url: 'https://pokeapi.co/api/v2/version-group/15/'
      }
    },
    {
      flavor_text: '나무열매를 여느 때보다\n빨리 사용한다.',
      language: { name: 'ko', url: 'https://pokeapi.co/api/v2/language/3/' },
      version_group: {
        name: 'x-y',
        url: 'https://pokeapi.co/api/v2/version-group/15/'
      }
    },
    {
      flavor_text: 'Permet d’utiliser plus rapidement\nune Baie tenue.',
      language: { name: 'fr', url: 'https://pokeapi.co/api/v2/language/5/' },
      version_group: {
        name: 'x-y',
        url: 'https://pokeapi.co/api/v2/version-group/15/'
      }
    },
    {
      flavor_text: 'Ermutigt zum frühzeitigen Einsatz\nvon Beeren.',
      language: { name: 'de', url: 'https://pokeapi.co/api/v2/language/6/' },
      version_group: {
        name: 'x-y',
        url: 'https://pokeapi.co/api/v2/version-group/15/'
      }
    },
    {
      flavor_text: 'Fomenta el uso rápido de la baya\nque porte.',
      language: { name: 'es', url: 'https://pokeapi.co/api/v2/language/7/' },
      version_group: {
        name: 'x-y',
        url: 'https://pokeapi.co/api/v2/version-group/15/'
      }
    },
    {
      flavor_text:
        'Favorisce l’uso anticipato di\nuna bacca tenuta dal Pokémon.',
      language: { name: 'it', url: 'https://pokeapi.co/api/v2/language/8/' },
      version_group: {
        name: 'x-y',
        url: 'https://pokeapi.co/api/v2/version-group/15/'
      }
    },
    {
      flavor_text: 'Makes the Pokémon use a held\nBerry earlier than usual.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'x-y',
        url: 'https://pokeapi.co/api/v2/version-group/15/'
      }
    },
    {
      flavor_text: 'きのみを　いつもより\n早く　使う。',
      language: { name: 'ja', url: 'https://pokeapi.co/api/v2/language/11/' },
      version_group: {
        name: 'x-y',
        url: 'https://pokeapi.co/api/v2/version-group/15/'
      }
    },
    {
      flavor_text: 'きの を　いつもより\nはやく　つかう。',
      language: {
        name: 'ja-Hrkt',
        url: 'https://pokeapi.co/api/v2/language/1/'
      },
      version_group: {
        name: 'omega-ruby-alpha-sapphire',
        url: 'https://pokeapi.co/api/v2/version-group/16/'
      }
    },
    {
      flavor_text: '나무열매를 여느 때보다\n빨리 사용한다.',
      language: { name: 'ko', url: 'https://pokeapi.co/api/v2/language/3/' },
      version_group: {
        name: 'omega-ruby-alpha-sapphire',
        url: 'https://pokeapi.co/api/v2/version-group/16/'
      }
    },
    {
      flavor_text: 'Permet d’utiliser plus rapidement\nune Baie tenue.',
      language: { name: 'fr', url: 'https://pokeapi.co/api/v2/language/5/' },
      version_group: {
        name: 'omega-ruby-alpha-sapphire',
        url: 'https://pokeapi.co/api/v2/version-group/16/'
      }
    },
    {
      flavor_text: 'Ermutigt zum frühzeitigen Einsatz\nvon Beeren.',
      language: { name: 'de', url: 'https://pokeapi.co/api/v2/language/6/' },
      version_group: {
        name: 'omega-ruby-alpha-sapphire',
        url: 'https://pokeapi.co/api/v2/version-group/16/'
      }
    },
    {
      flavor_text: 'Fomenta el uso rápido de la baya\nque porte.',
      language: { name: 'es', url: 'https://pokeapi.co/api/v2/language/7/' },
      version_group: {
        name: 'omega-ruby-alpha-sapphire',
        url: 'https://pokeapi.co/api/v2/version-group/16/'
      }
    },
    {
      flavor_text:
        'Favorisce l’uso anticipato di\nuna bacca tenuta dal Pokémon.',
      language: { name: 'it', url: 'https://pokeapi.co/api/v2/language/8/' },
      version_group: {
        name: 'omega-ruby-alpha-sapphire',
        url: 'https://pokeapi.co/api/v2/version-group/16/'
      }
    },
    {
      flavor_text: 'Makes the Pokémon use a held\nBerry earlier than usual.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'omega-ruby-alpha-sapphire',
        url: 'https://pokeapi.co/api/v2/version-group/16/'
      }
    },
    {
      flavor_text: 'きの を　いつもより\n早く　使う。',
      language: { name: 'ja', url: 'https://pokeapi.co/api/v2/language/11/' },
      version_group: {
        name: 'omega-ruby-alpha-sapphire',
        url: 'https://pokeapi.co/api/v2/version-group/16/'
      }
    },
    {
      flavor_text:
        'ＨＰが　すくなくなったら　たべる\nきのみを　ＨＰ　はんぶんの\nときに　たべてしまう。',
      language: {
        name: 'ja-Hrkt',
        url: 'https://pokeapi.co/api/v2/language/1/'
      },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text: 'HP가 줄어들면 먹을\n나무열매를 HP가\n절반일 때 먹어버린다.',
      language: { name: 'ko', url: 'https://pokeapi.co/api/v2/language/3/' },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        '原本ＨＰ變得很少時才會吃的樹果，\n在ＨＰ還有一半時就會把它吃掉。',
      language: {
        name: 'zh-Hant',
        url: 'https://pokeapi.co/api/v2/language/4/'
      },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        'Si le Pokémon tient une Baie à manger en cas de PV\nbas, il la mange dès qu’il a perdu la moitié de ses PV.',
      language: { name: 'fr', url: 'https://pokeapi.co/api/v2/language/5/' },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        'Setzt bestimmte Beeren nicht erst in einer Notlage\nein, sondern bereits dann, wenn seine KP auf die\nHälfte des Maximalwerts fallen.',
      language: { name: 'de', url: 'https://pokeapi.co/api/v2/language/6/' },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        'Cuando sus PS se ven reducidos a la mitad, engulle\nla baya que normalmente solo se comería cuando le\nquedasen pocos PS.',
      language: { name: 'es', url: 'https://pokeapi.co/api/v2/language/7/' },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        'Il Pokémon non attende di aver perso molti PS\nper mangiare certe bacche, ma lo fa non appena\ni suoi PS scendono a metà o meno.',
      language: { name: 'it', url: 'https://pokeapi.co/api/v2/language/8/' },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        'Makes the Pokémon eat a held Berry when its HP\ndrops to half or less, which is sooner than usual.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        'ＨＰが　少なくなったら　食べる\nきのみを　ＨＰ　半分の\n時に　食べてしまう。',
      language: { name: 'ja', url: 'https://pokeapi.co/api/v2/language/11/' },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        '原本ＨＰ变得很少时才会吃树果，\n在ＨＰ还有一半时就会把它吃掉。',
      language: {
        name: 'zh-Hans',
        url: 'https://pokeapi.co/api/v2/language/12/'
      },
      version_group: {
        name: 'sun-moon',
        url: 'https://pokeapi.co/api/v2/version-group/17/'
      }
    },
    {
      flavor_text:
        'ＨＰが　すくなくなったら　たべる\nきのみを　ＨＰ　はんぶんの\nときに　たべてしまう。',
      language: {
        name: 'ja-Hrkt',
        url: 'https://pokeapi.co/api/v2/language/1/'
      },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text: 'HP가 줄어들면 먹을\n나무열매를 HP가\n절반일 때 먹어버린다.',
      language: { name: 'ko', url: 'https://pokeapi.co/api/v2/language/3/' },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        '原本ＨＰ變得很少時才會吃的樹果，\n在ＨＰ還有一半時就會把它吃掉。',
      language: {
        name: 'zh-Hant',
        url: 'https://pokeapi.co/api/v2/language/4/'
      },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        'Si le Pokémon tient une Baie à manger en cas de PV\nbas, il la mange dès qu’il a perdu la moitié de ses PV.',
      language: { name: 'fr', url: 'https://pokeapi.co/api/v2/language/5/' },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        'Setzt bestimmte Beeren nicht erst in einer Notlage\nein, sondern bereits dann, wenn seine KP auf die\nHälfte des Maximalwerts fallen.',
      language: { name: 'de', url: 'https://pokeapi.co/api/v2/language/6/' },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        'Cuando sus PS se ven reducidos a la mitad, engulle\nla baya que normalmente solo se comería cuando le\nquedasen pocos PS.',
      language: { name: 'es', url: 'https://pokeapi.co/api/v2/language/7/' },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        'Il Pokémon non attende di aver perso molti PS\nper mangiare certe bacche, ma lo fa non appena\ni suoi PS scendono a metà o meno.',
      language: { name: 'it', url: 'https://pokeapi.co/api/v2/language/8/' },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        'Makes the Pokémon eat a held Berry when its HP\ndrops to half or less, which is sooner than usual.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        'ＨＰが　少なくなったら　食べる\nきのみを　ＨＰ　半分の\n時に　食べてしまう。',
      language: { name: 'ja', url: 'https://pokeapi.co/api/v2/language/11/' },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        '原本ＨＰ变得很少时才会吃树果，\n在ＨＰ还有一半时就会把它吃掉。',
      language: {
        name: 'zh-Hans',
        url: 'https://pokeapi.co/api/v2/language/12/'
      },
      version_group: {
        name: 'ultra-sun-ultra-moon',
        url: 'https://pokeapi.co/api/v2/version-group/18/'
      }
    },
    {
      flavor_text:
        'ＨＰが　すくなくなったら　たべる\nきのみを　ＨＰ　はんぶんの\nときに　たべてしまう。',
      language: {
        name: 'ja-Hrkt',
        url: 'https://pokeapi.co/api/v2/language/1/'
      },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text: 'HP가 줄어들면 먹을\n나무열매를 HP가\n절반일 때 먹어버린다.',
      language: { name: 'ko', url: 'https://pokeapi.co/api/v2/language/3/' },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text:
        '原本HP變得很少時才會吃的樹果，\n在HP還有一半時就會把它吃掉。',
      language: {
        name: 'zh-Hant',
        url: 'https://pokeapi.co/api/v2/language/4/'
      },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text:
        'Si le Pokémon tient une Baie à manger en cas de PV\nbas, il la mange dès qu’il a perdu la moitié de ses PV.',
      language: { name: 'fr', url: 'https://pokeapi.co/api/v2/language/5/' },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text:
        'Setzt bestimmte Beeren nicht erst in einer Notlage\nein, sondern bereits dann, wenn seine KP auf die\nHälfte des Maximalwerts fallen.',
      language: { name: 'de', url: 'https://pokeapi.co/api/v2/language/6/' },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text:
        'Cuando sus PS se ven reducidos a la mitad, engulle\nla baya que normalmente solo se comería cuando le\nquedasen pocos PS.',
      language: { name: 'es', url: 'https://pokeapi.co/api/v2/language/7/' },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text:
        'Il Pokémon non attende di aver perso molti PS\nper mangiare certe bacche, ma lo fa non appena\ni suoi PS scendono a metà o meno.',
      language: { name: 'it', url: 'https://pokeapi.co/api/v2/language/8/' },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text:
        'Makes the Pokémon eat a held Berry when its HP\ndrops to half or less, which is sooner than usual.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text:
        'ＨＰが　少なくなったら　食べる\nきのみを　ＨＰ　半分の\n時に　食べてしまう。',
      language: { name: 'ja', url: 'https://pokeapi.co/api/v2/language/11/' },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text: '原本HP变得很少时才会吃树果，\n在HP还有一半时就会把它吃掉。',
      language: {
        name: 'zh-Hans',
        url: 'https://pokeapi.co/api/v2/language/12/'
      },
      version_group: {
        name: 'lets-go-pikachu-lets-go-eevee',
        url: 'https://pokeapi.co/api/v2/version-group/19/'
      }
    },
    {
      flavor_text:
        'ＨＰが　すくなくなったら　たべる\nきのみを　ＨＰ　はんぶんの\nときに　たべてしまう。',
      language: {
        name: 'ja-Hrkt',
        url: 'https://pokeapi.co/api/v2/language/1/'
      },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text: 'HP가 줄어들면 먹을\n나무열매를 HP가\n절반일 때 먹어버린다.',
      language: { name: 'ko', url: 'https://pokeapi.co/api/v2/language/3/' },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text:
        '原本ＨＰ變得很少時才會吃的樹果，\n在ＨＰ還有一半時就會把它吃掉。',
      language: {
        name: 'zh-Hant',
        url: 'https://pokeapi.co/api/v2/language/4/'
      },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text:
        'Si le Pokémon tient une Baie à manger en cas de PV\nbas, il la mange dès qu’il a perdu la moitié de ses PV.',
      language: { name: 'fr', url: 'https://pokeapi.co/api/v2/language/5/' },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text:
        'Setzt bestimmte Beeren nicht erst in einer Notlage ein,\nsondern bereits dann, wenn seine KP auf die Hälfte des\nMaximalwerts fallen.',
      language: { name: 'de', url: 'https://pokeapi.co/api/v2/language/6/' },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text:
        'Cuando sus PS se ven reducidos a la mitad, engulle\nla baya que normalmente solo se comería cuando le\nquedasen pocos PS.',
      language: { name: 'es', url: 'https://pokeapi.co/api/v2/language/7/' },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text:
        'Il Pokémon non attende di aver perso molti PS\nper mangiare certe bacche, ma lo fa non appena\ni suoi PS scendono a metà o meno.',
      language: { name: 'it', url: 'https://pokeapi.co/api/v2/language/8/' },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text:
        'Makes the Pokémon eat a held Berry when its HP\ndrops to half or less, which is sooner than usual.',
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text:
        'ＨＰが　少なくなったら　食べる\nきのみを　ＨＰ　半分の\n時に　食べてしまう。',
      language: { name: 'ja', url: 'https://pokeapi.co/api/v2/language/11/' },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    },
    {
      flavor_text:
        '原本ＨＰ变得很少时才会吃树果，\n在ＨＰ还有一半时就会把它吃掉。',
      language: {
        name: 'zh-Hans',
        url: 'https://pokeapi.co/api/v2/language/12/'
      },
      version_group: {
        name: 'sword-shield',
        url: 'https://pokeapi.co/api/v2/version-group/20/'
      }
    }
  ],
  generation: {
    name: 'generation-iv',
    url: 'https://pokeapi.co/api/v2/generation/4/'
  },
  id: 82,
  is_main_series: true,
  name: 'gluttony',
  names: [
    {
      language: {
        name: 'ja-Hrkt',
        url: 'https://pokeapi.co/api/v2/language/1/'
      },
      name: 'くいしんぼう'
    },
    {
      language: { name: 'ko', url: 'https://pokeapi.co/api/v2/language/3/' },
      name: '먹보'
    },
    {
      language: {
        name: 'zh-Hant',
        url: 'https://pokeapi.co/api/v2/language/4/'
      },
      name: '貪吃鬼'
    },
    {
      language: { name: 'fr', url: 'https://pokeapi.co/api/v2/language/5/' },
      name: 'Gloutonnerie'
    },
    {
      language: { name: 'de', url: 'https://pokeapi.co/api/v2/language/6/' },
      name: 'Völlerei'
    },
    {
      language: { name: 'es', url: 'https://pokeapi.co/api/v2/language/7/' },
      name: 'Gula'
    },
    {
      language: { name: 'it', url: 'https://pokeapi.co/api/v2/language/8/' },
      name: 'Voracità'
    },
    {
      language: { name: 'en', url: 'https://pokeapi.co/api/v2/language/9/' },
      name: 'Gluttony'
    },
    {
      language: { name: 'ja', url: 'https://pokeapi.co/api/v2/language/11/' },
      name: 'くいしんぼう'
    },
    {
      language: {
        name: 'zh-Hans',
        url: 'https://pokeapi.co/api/v2/language/12/'
      },
      name: '贪吃鬼'
    }
  ],
  pokemon: [
    {
      is_hidden: true,
      pokemon: {
        name: 'bellsprout',
        url: 'https://pokeapi.co/api/v2/pokemon/69/'
      },
      slot: 3
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'weepinbell',
        url: 'https://pokeapi.co/api/v2/pokemon/70/'
      },
      slot: 3
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'victreebel',
        url: 'https://pokeapi.co/api/v2/pokemon/71/'
      },
      slot: 3
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'snorlax',
        url: 'https://pokeapi.co/api/v2/pokemon/143/'
      },
      slot: 3
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'shuckle',
        url: 'https://pokeapi.co/api/v2/pokemon/213/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'zigzagoon',
        url: 'https://pokeapi.co/api/v2/pokemon/263/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'linoone',
        url: 'https://pokeapi.co/api/v2/pokemon/264/'
      },
      slot: 2
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'gulpin',
        url: 'https://pokeapi.co/api/v2/pokemon/316/'
      },
      slot: 3
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'swalot',
        url: 'https://pokeapi.co/api/v2/pokemon/317/'
      },
      slot: 3
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'spoink',
        url: 'https://pokeapi.co/api/v2/pokemon/325/'
      },
      slot: 3
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'grumpig',
        url: 'https://pokeapi.co/api/v2/pokemon/326/'
      },
      slot: 3
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'munchlax',
        url: 'https://pokeapi.co/api/v2/pokemon/446/'
      },
      slot: 3
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'pansage',
        url: 'https://pokeapi.co/api/v2/pokemon/511/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'simisage',
        url: 'https://pokeapi.co/api/v2/pokemon/512/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'pansear',
        url: 'https://pokeapi.co/api/v2/pokemon/513/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'simisear',
        url: 'https://pokeapi.co/api/v2/pokemon/514/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'panpour',
        url: 'https://pokeapi.co/api/v2/pokemon/515/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'simipour',
        url: 'https://pokeapi.co/api/v2/pokemon/516/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'heatmor',
        url: 'https://pokeapi.co/api/v2/pokemon/631/'
      },
      slot: 1
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'skwovet',
        url: 'https://pokeapi.co/api/v2/pokemon/819/'
      },
      slot: 3
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'greedent',
        url: 'https://pokeapi.co/api/v2/pokemon/820/'
      },
      slot: 3
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'applin',
        url: 'https://pokeapi.co/api/v2/pokemon/840/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'flapple',
        url: 'https://pokeapi.co/api/v2/pokemon/841/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'appletun',
        url: 'https://pokeapi.co/api/v2/pokemon/842/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'rattata-alola',
        url: 'https://pokeapi.co/api/v2/pokemon/10091/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'raticate-alola',
        url: 'https://pokeapi.co/api/v2/pokemon/10092/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'raticate-totem-alola',
        url: 'https://pokeapi.co/api/v2/pokemon/10093/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'grimer-alola',
        url: 'https://pokeapi.co/api/v2/pokemon/10112/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'muk-alola',
        url: 'https://pokeapi.co/api/v2/pokemon/10113/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'slowpoke-galar',
        url: 'https://pokeapi.co/api/v2/pokemon/10164/'
      },
      slot: 1
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'zigzagoon-galar',
        url: 'https://pokeapi.co/api/v2/pokemon/10174/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'linoone-galar',
        url: 'https://pokeapi.co/api/v2/pokemon/10175/'
      },
      slot: 2
    },
    {
      is_hidden: true,
      pokemon: {
        name: 'snorlax-gmax',
        url: 'https://pokeapi.co/api/v2/pokemon/10206/'
      },
      slot: 3
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'flapple-gmax',
        url: 'https://pokeapi.co/api/v2/pokemon/10216/'
      },
      slot: 2
    },
    {
      is_hidden: false,
      pokemon: {
        name: 'appletun-gmax',
        url: 'https://pokeapi.co/api/v2/pokemon/10217/'
      },
      slot: 2
    }
  ]
}

export default gluttonyAbillity
